...
sealed trait Type {
  ...
  def presentation: String = this match {
    case SimpleType(c) => c
    case ParametrizedType(c, args) => c + args.map(_.presentation).mkString("[", ",", "]")
    case TupleType(types) => types.map(_.presentation).mkString("(", ",", ")")
    case FunctionType(x, y) => Seq(x.presentation, y.presentation).mkString("=>")
...
case class ParametrizedType(constructor: String, typeArgs: Seq[Type]) extends Type
