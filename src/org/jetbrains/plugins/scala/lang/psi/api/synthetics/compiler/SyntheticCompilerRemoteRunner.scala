 ...
 * @since  4/21/14.
 */
class SyntheticCompilerRemoteRunner(project: Project) extends RemoteResourceOwner {
  ...
  private val syntheticsJar = new File(canonicalLib, "scala-plugin-synthetics.jar")
  ...
  def compile(src: File, out: File, client: Client) {
    ...
    val cp = projectClassPath ++ Seq(reflectJar, compilerJar, libraryJar, runnersJar, syntheticsJar, compilerSettingsJar, out)
