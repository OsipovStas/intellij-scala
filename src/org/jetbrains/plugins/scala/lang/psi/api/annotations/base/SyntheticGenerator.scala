...
object SyntheticGenerator {
  ...
  def apply(macros: MacroAnnotation): SyntheticGenerator = apply(macros.definitions, macros.annotations)

  def apply(definitions: Seq[SyntheticDefinitions], annotations: Seq[String]) = new SyntheticGenerator {

    override val creators: Seq[SyntheticCreators] = {
      definitions.map(SyntheticCreators(_))
    }
